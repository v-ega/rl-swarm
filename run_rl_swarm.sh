...
start_modal_login() {
  ...
  if ! need_cmd node; then
    echo_yellow "Node.js not found → install NVM + Node.js"
  ...
  if ! need_cmd yarn; then
    echo_yellow "Yarn not found → npm i -g yarn"
...
wait_api_key_activation() {
  if [[ "${REQUIRE_API_KEY_ACTIVATION}" != "1" ]]; then
    echo_yellow ">> API-key activation check skipped (REQUIRE_API_KEY_ACTIVATION=0)."
    return 0
  fi

  if [[ -z "${ORG_ID:-}" || "$ORG_ID" == "null" ]]; then
    echo_yellow ">> API-key activation check skipped (ORG_ID empty)."
    return 0
  fi
  ...
  while true; do
    ...
    if (( $(date +%s) >= deadline )); then
      echo_yellow ">> Activation wait timed out — proceeding anyway."
      break
    fi
  ...
  if [[ -f "$DST" ]]; then
    if ! cmp -s "$SRC" "$DST"; then
      if [[ -n "$GENSYN_RESET_CONFIG" ]]; then
        ...
        echo_yellow ">> Config differs. Keep existing (set GENSYN_RESET_CONFIG to overwrite)."
      fi
    fi
  else
    cp "$SRC" "$DST"
